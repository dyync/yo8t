    ...
    with gr.Blocks() as app:
                    ...
                    with gr.Column(scale=4):
                        with gr.Accordion(("Load vLLM Parameters"), open=True) as acc_load:
                            vllm_load_components = VllmLoadComponents(
                                ...
                                tensor_parallel_size=gr.Number(1, 8, value=2, label="tensor_parallel_size", info=f"Number of tensor parallel replicas."),
        ...
        with gr.Tabs() as fun_tabs:
            with gr.TabItem("Text", id=0):
                with gr.Row() as vllm_prompt_input:
            ...
            with gr.TabItem("Audio", id=1):
                with gr.Row(visible=True) as row_audio:
                    ...
                    with gr.Column(scale=1):
                        with gr.Row() as row_audio_output:
                            audio_path = gr.Textbox(visible=True)
                            audio_output = gr.Textbox(label="Transcription", lines=8)
                        with gr.Row() as row_audio_transcribe:
                            audio_transcribe_btn = gr.Button("Audio Transcribe")
            ...
            with gr.TabItem("Image", id=2):
                ...
                with gr.Row(visible=True) as row_image_prompt:
                    image_prompt = gr.Textbox(placeholder=f'Two dragons fighting in space', value=f'Two dragons fighting in space', label="Prompt", show_label=True, visible=True, lines=2)
                with gr.Row() as row_image_generate:
                    image_generate_btn = gr.Button("Image generate")
            ...
            with gr.TabItem("Video", id=3):
                ...
                with gr.Row() as row_video_generate:
                    btn_video_generate = gr.Button("Video generate")
            ...
            with gr.TabItem("Interface", id=4):
